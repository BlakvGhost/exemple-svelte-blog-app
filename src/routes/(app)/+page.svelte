...
<div class="">
				...
				<div class="mx-4 w-full md:w-2/4">
					...
					<div class="my-6">
						<button
							...
							transition hover:border-primary-900 hover:bg-primary-900">Continue Reading</button
						>
					</div>
				</div>
						...
						<div class="my-2">
							<div class="flex w-full flex-col gap-3 overflow-auto">
								{#each Array(2) as _, index}
									<Post type={true} custom_class="w-full bg-primary-600 border-0 text-white"></Post>
								{/each}
							</div>
						</div>
					</div>
	...
	<div class="md:-p container mx-auto block px-3 md:hidden">
		...
		<div class="my-4 flex flex-col items-center gap-4">
			{#each Array(2) as _, index}
				<Post type={true} custom_class="bg-primary-600 w-full border-0 text-white"></Post>
			{/each}
		</div>
	</div>
