...
<div
	class="app flex min-h-screen w-full flex-col bg-cover bg-no-repeat"
	style="background-image: url('{bgAuthImage}');"
>
	<main class="mx-auto my-0 flex w-full flex-1 flex-col p-2 text-white">
		<div
			class="flex max-h-screen w-full flex-col items-center justify-center overflow-hidden sm:px-4"
		>
			<div
				...
				<div class="space-y-6 p-4 py-6 sm:px-4 sm:py-0">
					<div class="grid grid-cols-1 gap-x-3 sm:px-6">
						...
						>
							...
							{/if}
							{#if isPage()}
								<span class="mx-2 font-bold">Sign ui with Google</span>
							{/if}
						</button>
					</div>
					<div class="relative">
